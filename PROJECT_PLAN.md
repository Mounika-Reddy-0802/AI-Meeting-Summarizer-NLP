...
## 10. Prompts for an AI coding agent (Claude Code / Cursor)

**Common header — paste first, then the member's block.**
```
This is a new project built from scratch: an academic NLP meeting summarizer. Deepgram does
speech-to-text; OUR OWN fine-tuned Hugging Face seq2seq model does summarization; action-item
extraction and semantic search are our own code. No LLM API in the app path. Read GIT_RULES.md and
PROJECT_PLAN.md first. I am <NAME>; modify only my owned folders (PROJECT_PLAN.md §1) unless I say
otherwise. Work on branch week<N>-<name>-<topic>. Never write secrets; .env.example holds names only.
Commit messages follow GIT_RULES.md §7: short lowercase imperative, one logical change each,
numbers in the message when a number changed. After each task print files changed, how to run, and
what to test manually. Do one numbered task at a time and stop for my confirmation.
```

**Mounika — Day 0 (run in the empty folder)**
```
Do PROJECT_PLAN.md §4 "Mounika — create the repository": git init, the root files with the exact
.gitignore and PR template contents, and the folder skeleton from §2 with a one-line README.md in
each empty folder. No application code yet. One commit per root file, one for the skeleton. Print
the tree and git log when done.
```

**Mounika — Week 1**
```
Build backend/ per PROJECT_PLAN.md §5 tasks 1–7 in order, implementing exactly the API contract in
§3. Deepgram is called server-side only via the prerecorded endpoint with diarization. Stub
summarize.py, action_items.py and search.py with TODOs so the upload pipeline runs end to end.
```

**Krishna — Week 1**
```
Build frontend/ per PROJECT_PLAN.md §5 tasks 1–6 in order against the API contract in §3. Include
a json-server mock (frontend/mock/db.json) so the UI runs without the backend. Recording uses
MediaRecorder and POSTs the blob to /meetings/upload; there is no transcription key in the frontend.
TypeScript strict; npm run build must pass.
```

**Lahari — Week 1**
```
Build ml/ per PROJECT_PLAN.md §5 tasks 1–5, designed so all training runs on Kaggle, never
locally: train.py must support --push_to_hub and --resume_from_checkpoint, and
ml/kaggle/train_samsum.ipynb must pull ml/, install, train and push to HF Hub using the HF_TOKEN
Kaggle secret. Then implement backend/app/services/summarize.py loading the checkpoint from
MODEL_PATH at startup (single pass; chunking is next week).
```

**Week 2 / Week 3** — use the bullet lists in §6 and §7 for the member as the task list, with the same header.
